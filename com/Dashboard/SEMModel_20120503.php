...
class SEMModel extends SQLData{
	...
	function getLastUpdate($id){
		$this->open(0);
		$q=	"SELECT DATE_FORMAT(c.range_date,'%d %M %Y') AS lastUpdate, c.range_date AS end_date, DATE_FORMAT(c.range_date,'%Y-%m') AS monthLast
...
// 		}
		for ($i=0;$i<sizeof($data);$i++){
			$temp += intval($data[$i]['clicks']) + intval($data2[$i]['clicks']);
		}
		$data =null;
...
//  		var_dump($kpi);exit;
	...
	function getClickThisMonth($id){
		...
		$q = "SELECT SUM(click) AS this_month FROM tbl_semad_report r LEFT JOIN tbl_project p ON p.id=r.semID WHERE DATE_FORMAT(range_date,'%Y-%m') ='".$last['monthLast']."' AND  semID = '".$id."'";
		$this->open(0);
		$r = $this->fetch($q);
		$this->close();
	...
	function getCTRFromStart($id, $kpi){
		$last = $this->getLastUpdate($id);
		$this->open(0);
		$q= "SELECT SUM(c.clicks) AS total_click, SUM(c.impression)  AS total_impression
				FROM tbl_semad_summary c LEFT JOIN tbl_project k ON c.semID = k.id
				WHERE c.description = 'Total' AND c.range_date = '".$last['end_date']."' AND c.semID = '".$id."'";
...
// 				WHERE  r.range_date >= p.start_date AND  r.semID = '".$id."';";
		$r = $this->fetch($q);
		$this->close();
...
// 		$percent = ((($ctr)/$kpi)*100);
	...
	function getTotalCostThisMonth($id, $cpc){
		...
		$q = "SELECT c.cost, c.clicks 
			...
			WHERE c.semID = '".$id."' AND c.range_date = '".$last['end_date']."'";
...
// 			WHERE  semID = '".$id."' AND r.range_date >= s.start_date;";
	...
	function getCPCPerDay($id){
		$this->open();
		$q = "SELECT cost, click, range_date FROM tbl_semad_report JOIN tbl_project WHERE  semID = '".$id."' AND tbl_semad_report.range_date >= tbl_project.start_date AND  range_date >= DATE_SUB(CURRENT_DATE, INTERVAL 10 DAY) GROUP BY range_date,campaign";
